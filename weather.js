...
async function weatherSearch(city) {
  let firstSearch = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${city},india&units=metric&APPID=474b8c611d11180ad3d73644d069118f`
  );
  ...
  document.querySelector("#pres").textContent =
  ...
  document.querySelector("#mapDiv").innerHTML = `
    <iframe src="https://maps.google.com/maps?width=100%25&amp;height=600&amp;hl=en&amp;q=${city}&amp;t=&amp;z=14&amp;ie=UTF8&amp;iwloc=B&amp;output=embed"></iframe>
    
    `;
}

async function forecast(city) {
  let secSearch = await fetch(
    `https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=metric&appid=474b8c611d11180ad3d73644d069118f`
  );
